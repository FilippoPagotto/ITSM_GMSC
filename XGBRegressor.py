...
selected_features_corr = SelectKBest(f_regression, k=5).fit(X_train, y_train).get_support()
...
selected_features_anova = SelectKBest(f_classif, k=5).fit(X_train, y_train).get_support()
...
# Create and train the XGBoost model using correlation-based features
model_corr = XGBRegressor(n_estimators=100, random_state=42)
model_corr.fit(X_train_selected_corr, y_train, eval_metric="rmse", eval_set=[(X_test_selected_corr, y_test)], early_stopping_rounds=10, verbose=True)

# Create and train the XGBoost model using ANOVA-based features
model_anova = XGBRegressor(n_estimators=100, random_state=42)
model_anova.fit(X_train_selected_anova, y_train, eval_metric="rmse", eval_set=[(X_test_selected_anova, y_test)], early_stopping_rounds=10, verbose=True)
